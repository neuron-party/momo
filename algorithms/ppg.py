...
# auxiliary head for value approximation
# can just use cleanrl's model implementation
...
class PPO4:
    ...
    def get_action(self, state):
        ...
        return action.detach().cpu().numpy(), log_probs.detach().cpu().numpy(), value.reshape(-1, ).detach().cpu().numpy(), pi.detach().cpu().numpy()
    ...
    def _make_policy_dataset(self, next_state):
        ...
        self.n_pi_index = (self.n_pi_index + 1) % 32
    ...
    def _make_auxiliary_dataset(self):
        ...
        with torch.no_grad():
            for i, state in enumerate(self.auxiliary_states.transpose(1, 0, 2, 3, 4)):
                ...
                pi = self.model(state) # replace this later
                self.auxiliary_pi[:, i] = pi.detach().cpu().numpy().copy()
